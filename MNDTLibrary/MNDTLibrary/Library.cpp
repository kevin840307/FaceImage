...
namespace MNDT {
	...
	void Inner::L1NormalizedHistogram8bit(std::vector<double>& histogram
		, C_UINT32 start, C_UINT32 size)
	{
		double L1 = 0;
		C_UINT32 end = start + size;
	...
	void Screenshot8bit(const Image& srcImage, UCHAR* pur
		, const Rect& rect)
	{
		for (UINT32 row = rect.Y(); row <= rect.EndY(); row++)
		{
			for (UINT32 col = rect.X(); col <= rect.EndX(); col++)
			{
				*pur = srcImage.image[row][col];
				pur++;
			}
		}
	}
		...
		, C_UINT32 width, C_UINT32 height)
	{
		...
		for (UINT32 index = 1; index < labelLinks.size(); index++)
		{
			...
			if (rect.Width() > 0 && rect.Height() > 0)
			{
				...
				delete[] rectData2;
				rectData2 = nullptr;
				MNDT::DrawRect8bit(purImage, rects[index], 255);
			}
		}
	...
	void ThresholdingOTSU(C_UCHAR* src, UCHAR* pur
		, C_UINT32 width, C_UINT32 height)
	{
		...
		, C_UINT32 block)
	{
		...
		for (UINT32 row = offset; row < height - offset; row++)
		{
