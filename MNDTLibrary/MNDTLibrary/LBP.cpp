...
void LBP::LBPHistogram(C_UCHAR* src
	, C_UINT32 width, C_UINT32 height
...
void LBP::LBPHistogram(C_UCHAR* src, std::vector<double>& histogram
	, C_UINT32 width, C_UINT32 height
	, C_UINT32 gridX, C_UINT32 gridY
	, C_UINT32 bin)
{
	// 1. init params
	Image srcImage(const_cast<UCHAR*>(src), width, height, MNDT::ImageType::GRAY_8BIT);
	C_UINT32 cellWidth = static_cast<UINT32>(ceil(static_cast<float>(width) / gridX));
	C_UINT32 cellHeight = static_cast<UINT32>(ceil(static_cast<float>(height) / gridY));
	...
	for (UINT32 index = 0; index < histogramSize; index++)
	{
		MNDT::Inner::L1NormalizedHistogram8bit(histogram, index * bin, bin);
	}
}