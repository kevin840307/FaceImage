...
namespace MNDTVisualization
{
    ...
    class MNDTLibrary
    {
        private const string DLL_PATH = @"D:\GitHub\FaceImage\MNDTVisualization\MNDTVisualization\bin\Debug\MNDTLibrary.dll";
        //private const string DLL_PATH = @"D:\services6\Image\MNDTLibrary\x64\Debug\MNDTLibrary.dll";
        ...
        [DllImport(DLL_PATH)]
        unsafe private static extern void mndtFaceSearch(IntPtr src, IntPtr pur
            , int width, int height
            , float sigma, float threshold, int minSize
            , int* numCss);

        public Bitmap FaceSearch(Bitmap srcImage
            , float sigma, float threshold, int minSize)
        {
            Bitmap purImage = new Bitmap(srcImage.Width, srcImage.Height, PixelFormat.Format24bppRgb);
            Rectangle size = new Rectangle(0, 0, srcImage.Width, srcImage.Height);
        ...
        [DllImport(DLL_PATH)]
        unsafe private static extern void mndtLBPHistogram(IntPtr src
           , int width, int height
           , int gridX, int gridY
           , int bin);
        ...
        [DllImport(DLL_PATH)]
        unsafe private static extern void mndtContourFrame8bit(IntPtr src, IntPtr pur
             , int width, int height);

        public Bitmap ContourFrame8bit(Bitmap srcImage)
        {
            Bitmap purImage = new Bitmap(srcImage.Width, srcImage.Height, PixelFormat.Format8bppIndexed);
            Rectangle size = new Rectangle(0, 0, srcImage.Width, srcImage.Height);
            BitmapData srcData = srcImage.LockBits(size, ImageLockMode.ReadOnly, PixelFormat.Format8bppIndexed);
            ...
            unsafe
            {
                IntPtr srcPtr = srcData.Scan0;
                IntPtr purPtr = purData.Scan0;
                mndtContourFrame8bit(srcPtr, purPtr
                    , srcImage.Width, srcImage.Height);
            }
        ...
        [DllImport(DLL_PATH)]
        unsafe private static extern void mndtMarkLine(IntPtr src, IntPtr pur
             , int width, int height);

        public Bitmap MarkLine(Bitmap srcImage)
        {
            Bitmap purImage = new Bitmap(srcImage.Width, srcImage.Height, PixelFormat.Format24bppRgb);
            Rectangle size = new Rectangle(0, 0, srcImage.Width, srcImage.Height);
            BitmapData srcData = srcImage.LockBits(size, ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
        ...
        [DllImport(DLL_PATH)]
        unsafe private static extern void mndtThresholdingOTSU(IntPtr src, IntPtr pur
            , int width, int height);

        public Bitmap ThresholdingOTSU(Bitmap srcImage)
        {
            Bitmap purImage = new Bitmap(srcImage.Width, srcImage.Height, PixelFormat.Format8bppIndexed);
            Rectangle size = new Rectangle(0, 0, srcImage.Width, srcImage.Height);
            BitmapData srcData = srcImage.LockBits(size, ImageLockMode.ReadOnly, PixelFormat.Format8bppIndexed);
            ...
            unsafe
            {
                IntPtr srcPtr = srcData.Scan0;
                IntPtr purPtr = purData.Scan0;
                mndtThresholdingOTSU(srcPtr, purPtr
                    , srcImage.Width, srcImage.Height);
            }
        ...
        [DllImport(DLL_PATH)]
        unsafe private static extern void mndtExpansion8bit(IntPtr src, IntPtr pur
            , int width, int height
            , int block);

        public Bitmap Expansion8bit(Bitmap srcImage, int block)
        {
            Bitmap purImage = new Bitmap(srcImage.Width, srcImage.Height, PixelFormat.Format8bppIndexed);
            Rectangle size = new Rectangle(0, 0, srcImage.Width, srcImage.Height);
            BitmapData srcData = srcImage.LockBits(size, ImageLockMode.ReadOnly, PixelFormat.Format8bppIndexed);
            ...
            unsafe
            {
                IntPtr srcPtr = srcData.Scan0;
                IntPtr purPtr = purData.Scan0;
                mndtExpansion8bit(srcPtr, purPtr
                    , srcImage.Width, srcImage.Height
                    , block);
            }
        ...
        [DllImport(DLL_PATH)]
        unsafe private static extern void mndtErosion8bit(IntPtr src, IntPtr pur
            , int width, int height
            , int block);

        public Bitmap Erosion8bit(Bitmap srcImage, int block)
        {
            Bitmap purImage = new Bitmap(srcImage.Width, srcImage.Height, PixelFormat.Format8bppIndexed);
            Rectangle size = new Rectangle(0, 0, srcImage.Width, srcImage.Height);
            BitmapData srcData = srcImage.LockBits(size, ImageLockMode.ReadOnly, PixelFormat.Format8bppIndexed);
